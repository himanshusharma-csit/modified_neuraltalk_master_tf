...
from Utils.Dataset import split_dataset, prepare_testing_dataset, load_vocabulary
...
image_features_path = configurations["image_features_path"]
...
# ------------------------------------------------------------------------------------------------------------------
# 1.0 (OPTIONAL) INPUT THE IMAGE TO BE CAPTIONED FROM THE USER
# ------------------------------------------------------------------------------------------------------------------
...
_, _, testing_image_names = split_dataset(list(processed_image_captions.keys()))
# Extract the already saved images features of the testing images
testing_image_features = prepare_testing_dataset(image_features_path, testing_image_names)

# ------------------------------------------------------------------------------------------------------------------
# 2.0 INITIALIZE THE PRE-SAVED FEATURE ENCODER AND MULTIMODAL DECODER
# ------------------------------------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------------------------------------
# 3.0 LOAD INITIALIZE THE PREDICTIONS INSTANCE WITH ALL THE NECESSARY CONFIGURATIONS AND GENERATE CAPTIONS
# ------------------------------------------------------------------------------------------------------------------
prediction_manger = generate_prediction_manager(configurations, feature_encoder, decoder)
predicted_caption = prediction_manger.predict(testing_image_features[0], load_vocabulary(vocabulary_path))
image_absolute_path = os.path.join(os.path.abspath("D:\modified_neuraltalk_master_tf\Data\Flickr_8K\Images"), testing_image_names[0])
image = Image.open(image_absolute_path)
image.show()
print(
        '\n------------------------------------------------------------------------------------------------------------------')
print("Original Caption: ", processed_image_captions[testing_image_names[0]][0])
